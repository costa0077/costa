...
btnEnviar.onclick = function(){
    if (nome.value.trim() === '' || quantidade.value.trim() === '' || preco.value.trim() === '' || prateleira.value.trim() === '' || descricao.value.trim() === '' || categoria.value.trim() === '') {
...
btnExcluir.onclick = function(){
    for (let i = 0; i < BD.length; i++){
        ...
        if (elemento.checked){
            ...
            tabela.innerHTML = `<tr><td width="60px"></td><td>Nome</td><td>Quant.</td><td>Preço</td></tr>`;
            montarTabela();
        }
    }
}
...
function montarTabela(){
    for (let i = 0; i < BD.length; i++){
        tabela.innerHTML += `<tr><td width="60px"><input type="checkbox" id="${i}" onchange="verificar()"></td><td>${BD[i].nome}</td><td>${BD[i].quantidade}</td><td>${BD[i].preco}</td><td>${BD[i].prateleira}</td><td>${BD[i].descricao}</td><td><${BD[i].categoria}</td></tr>`;
    }
}
